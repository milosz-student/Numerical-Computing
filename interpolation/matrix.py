...
def interchangeRows(matrix, k, index, start, end):
    matrix[k][start:end], matrix[index][start:end] = matrix[index][start:end], matrix[k][start:end]


def CreateDiagonalMatrixByVector(v):
    n = len(v)
    tmp = createMatrix(n, n, 0)
    for i in range(n):
        tmp[i][i] = v[i]
    return tmp
...
def FactorLU(A, b):
    ...
    tmp = createMatrix(m, m, 1)
    tmp = tmp[0]
    L = CreateDiagonalMatrixByVector(tmp)
    U = createMatrix(m, m, 0)
    for j in range(m):
        for i in range(j + 1):
            U[i][j] += A_copy[i][j]
            for k in range(i):
...
def subVector(A, B):
    # sub two vectors, working ONLY when they have same size
    total = []
    for i in range(len(A)):
